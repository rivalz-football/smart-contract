...
describe("rivalz-goalflip", () => {
    ...
    it("should play the game", async () => {
        try {
            ...
            const tx = await program.methods
                .play(Position.Forward, Corner.TopLeft, new anchor.BN(LAMPORTS_PER_SOL / 2))
    ...
    it("should result the game match", async () => {
        try {
            ...
            let playedGameMatch = await program.account.gameMatch.fetch(gameMatch.publicKey.toBase58());
