...
describe("rivalz-goalflip", () => {
  ...
  it("should play the game", async () => {
    try {
      const initGameMatchContext = {
        game: "EUFn92GrwUu18hUQbTJ1TZowG3pB3vkTsbaArAYM2xdz",
      ...
      console.log({
        initGameMatchContext,
      });

      const tx = await program.methods
        .play(Position.Forward, Corner.TopLeft, new anchor.BN(LAMPORTS_PER_SOL))
        .accounts(initGameMatchContext)
  ...
  it("should result the game match", async () => {
    try {
      const initResultGameMatchContext = {
        game: "EUFn92GrwUu18hUQbTJ1TZowG3pB3vkTsbaArAYM2xdz",
        gameMatch: gameMatch.publicKey.toBase58(),
      ...
      console.log({
        initResultGameMatchContext,
      });

      const result_match = await program.methods
        .resultGameMatch()
        .accounts(initResultGameMatchContext)
        .rpc();
